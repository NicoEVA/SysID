...
for k in range(0, N):
    ...
    z_k1_k      = kf_calc_h(0, x_k1_k, U_k[:,k])                         # prediction of observation (for validation)   
...
print("EKF state estimation error RMS = %.3E, completed run with %d samples in %.2f seconds." %(np.sqrt((np.square(EstErr_x)).mean()), N, t1-t0))  
